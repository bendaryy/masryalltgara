                ...
                <table id="example2" class="table table-striped table-bordered text-center">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('showDraftDetails',$draft->id) }}" class="btn btn-secondary">عرض
                                    التفاصيل</a>
                                @if ($draft->inv_uuid == null)
                                <a href="{{ route('editinvoice', $draft->id) }}"
                                    class="btn btn-sm btn-info radius-2">تعديل الفاتورة</a>
                                @else
                                <a class="btn btn-sm btn-info radius-2" style="opacity: 0.5;cursor: text;"
                                    disabled>تعديل الفاتورة</a>
                                @endif
                            </td>
                            ...
                            <td>
                                <form action="{{ route('deleteDraft',$draft->id) }}" method="post">
                                    ...
                                    <button class="btn btn-danger"
                                        onclick="return confirm('هل أنت متأكد من مسح الفاتورة؟');">مسح الفاتورة</button>
                                </form>
                            </td>
                        </tr>
                        @endforeach
